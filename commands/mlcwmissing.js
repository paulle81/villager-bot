...
exports.run = async (client, message) => {
    ...
    const sheetResult = await mlcwSheetSchema.findOne({ _id: `mlcw${guild.id}` });

    const clanTag = result.clanTag;
    const mlcwSheet = sheetResult.mlcwSheet;
    ...
    const range = `'${sheet || 'Sheet1'}'!D17:F`;
    ...
    const gsrun = async (cl) => {
        ...
        try {
            ...
            if (data.values.length) {
                ...
                rostered = rostered.filter((player, index, self) => self.findIndex(t => t.tag === player.tag && t.name === player.name) === index);
                rostered.pop();
                ...
                const exampleEmbed = new MessageEmbed()
                    ...
                    .addFields(
                        {
                            name: 'Player',
                            value: notIn.length > 0 ? notIn.map(e => {
